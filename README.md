...
```bash
curl -fsSL https://raw.githubusercontent.com/TenCuaBan/telegram-agy/main/install.sh | bash
```

*(Lưu ý: Thay thế URL trên bằng đường link thực tế đến file `install.sh` của bạn trên GitHub sau khi bạn publish mã nguồn).*
...
1. Clone mã nguồn:
   ```bash
   git clone https://github.com/TenCuaBan/telegram-agy.git
   cd telegram-agy
   ```
